    ...
    class RichTextBoxHelper : DependencyObject
    {
        ...
        public static void SetRichTextBox(RichTextBox richTextBox, L.Section tag, string signature, DateTime creationTime, string comment, Anchor anchor, bool isTrust)
        {
...

            {
                ...
                Run r = new Run();
                if (isTrust) r.Foreground = new SolidColorBrush(App.LairColors.Message_Trust);
                else r.Foreground = new SolidColorBrush(App.LairColors.Message_Untrust);
                r.Text = signature;
            ...
            if (anchor != null)
            {
                ...
                if (wrapper != null)
                {
...

                    {
                        ...
                        Run r = new Run();
                        if (wrapper.IsTrust) r.Foreground = new SolidColorBrush(App.LairColors.Message_Trust);
                        else r.Foreground = new SolidColorBrush(App.LairColors.Message_Untrust);
