...
namespace Outopos.Windows
{
    ...
    partial class ChatMessageEditWindow : Window
    {
        ...
        private void _tabControl_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (_tabControl.SelectedItem == _previewTabItem)
            {
                ...
                else
                {
                    ...
                    if (comment.Length > ChatMessageContent.MaxCommentLength)
                    {
                        comment = comment.Substring(0, ChatMessageContent.MaxCommentLength);
                    }
        ...
        {
            if (string.IsNullOrWhiteSpace(_commentTextBox.Text) || _commentTextBox.Text.Length > ChatMessageContent.MaxCommentLength)
            {
            ...
            {
                _countLabel.Content = string.Format("{0} / {1}", _commentTextBox.Text.Length, ChatMessageContent.MaxCommentLength);
            }
        }
