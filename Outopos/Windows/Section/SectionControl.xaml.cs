...
namespace Outopos.Windows
{
    ...
    partial class SectionControl : UserControl
    {
        ...
        public SectionControl(OutoposManager outoposManager, BufferManager bufferManager)
        {
            ...
            try
            {
                ...
                for (; ; )
                {
                    ...
                    if (!refreshStopwatch.IsRunning || refreshStopwatch.Elapsed.TotalSeconds >= 20)
                    {
                        ...
                        {
                            var sectionProfileInfos = new List<SectionProfileInfo>();

                            {
                                SectionProfileInfo leaderSectionProfileInfo;

                                {
                                    var header = _outoposManager.GetSectionProfileHeaders(sectionTreeItem.Tag)
                                        .FirstOrDefault(n => n.Certificate.ToString() == sectionTreeItem.LeaderSignature);
                                    if (header == null) continue;

                                    var content = _outoposManager.Download(header).Result;
                                    if (content == null) continue;

                                    leaderSectionProfileInfo = new SectionProfileInfo()
                                    {
                                        Header = header,
                                        Content = content,
                                    };
                                ...
                                sectionProfileInfos.Add(leaderSectionProfileInfo);

                                var hashset = new HashSet<string>();

                                hashset.Add(sectionTreeItem.LeaderSignature);

                                for (int i = 0; i < sectionProfileInfos.Count; i++)
                                {
                                    foreach (var trustSignature in sectionProfileInfos[i].Content.TrustSignatures)
                                    {
                                        if (hashset.Contains(trustSignature)) continue;
                                        hashset.Add(trustSignature);

                                        SectionProfileInfo info;

                                        {
                                            var header = _outoposManager.GetSectionProfileHeaders(sectionTreeItem.Tag)
                                                .FirstOrDefault(n => n.Certificate.ToString() == trustSignature);
                                            if (header == null) continue;

                                            var content = _outoposManager.Download(header).Result;
                                            if (content == null) continue;

                                            info = new SectionProfileInfo()
                                            {
                                                Header = header,
                                                Content = content,
                                            };
                                        ...
                                        if (info != null)
                                        {
                                            sectionProfileInfos.Add(info);
                                        }
                                        else
                                        {
                                            var cachedInfo = sectionTreeItem.TrustSectionProfileInfos
                                                .FirstOrDefault(n => n.Header.Certificate.ToString() == trustSignature);

                                            if (cachedInfo != null)
                                            {
                                                sectionProfileInfos.Add(cachedInfo);
                                            }
                                        }
                                    }
                                }
                            ...
                            {
                                lock (sectionTreeItem.TrustSectionProfileInfos.ThisLock)
                                {
                                    sectionTreeItem.TrustSectionProfileInfos.Clear();
                                    sectionTreeItem.TrustSectionProfileInfos.AddRange(sectionProfileInfos);
                                }
                            }
                        }
                    }
        ...
        private void Update()
        {
            ...
            _mainWindow.Title = string.Format("Outopos {0}", App.OutoposVersion);
            _refresh = true;
        }
        ...
        private void Update_Title()
        {
            ...
            if (_mainWindow.SelectedTab == MainWindowTabType.Section)
            {
                if (_treeView.SelectedItem is SectionCategorizeTreeViewItem)
                {
                    ...
                    _mainWindow.Title = string.Format("Outopos {0} - {1}", App.OutoposVersion, selectTreeViewItem.Value.Name);
                }
                else if (_treeView.SelectedItem is SectionTreeViewItem)
                {
                    var selectTreeViewItem = (SectionTreeViewItem)_treeView.SelectedItem;

                    _mainWindow.Title = string.Format("Outopos {0} - {1}", App.OutoposVersion, MessageConverter.ToSectionString(selectTreeViewItem.Value.Tag));
                }
            }
        }
        ...
        private void _sectionCategorizeTreeViewItemNewSectionMenuItem_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (Settings.Instance.Global_DigitalSignatureCollection.Count > 0)
            {
                ...
                if (newSectionwindow.ShowDialog() == true)
                {
                    ...
                    var tag = new Section(newSectionwindow.SectionName, id);
                    var sectionTreeItem = new SectionTreeItem(tag);
                    sectionTreeItem.LeaderSignature = newSectionwindow.LeaderSignature;

                    SectionTreeItemEditWindow sectionTreeItemEditWindow = new SectionTreeItemEditWindow(sectionTreeItem, _outoposManager, _bufferManager);
        ...
        private void _sectionCategorizeTreeViewItemCopyInfoMenuItem_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                sb.AppendLine(OutoposConverter.ToSectionString(item.Value.Tag, item.Value.LeaderSignature));
        ...
        private void _sectionTreeViewItemEditMenuItem_Click(object sender, RoutedEventArgs e)
        {
            ...
            SectionTreeItemEditWindow sectionTreeItemEditWindow = new SectionTreeItemEditWindow(selectTreeViewItem.Value, _outoposManager, _bufferManager);
            sectionTreeItemEditWindow.Owner = _mainWindow;
        ...
        private void _sectionTreeViewItemCopyInfoMenuItem_Click(object sender, RoutedEventArgs e)
        {
            ...
            sb.AppendLine(OutoposConverter.ToSectionString(selectTreeViewItem.Value.Tag, selectTreeViewItem.Value.LeaderSignature));
        ...
        private void _sectionTreeViewItemTrustSignaturesPreviewMenuItem_Click(object sender, RoutedEventArgs e)
        {
            ...
            TrustSignaturesPreview window = new TrustSignaturesPreview(signatureTreeItem);
            window.Owner = _mainWindow;
            window.ShowDialog();
        }
        ...
        {
            Dictionary<string, SectionProfileInfo> dic = new Dictionary<string, SectionProfileInfo>();

            foreach (var item in sectionTreeItem.TrustSectionProfileInfos)
            {
                dic[item.Header.Certificate.ToString()] = item;
            }
            ...
            {
                SectionProfileInfo leaderSectionProfileInfo;
                if (!dic.TryGetValue(sectionTreeItem.LeaderSignature, out leaderSectionProfileInfo)) return null;

                workSignatureTreeItems.Add(new SignatureTreeItem(leaderSectionProfileInfo));
            ...
            {
                var sortList = workSignatureTreeItems.SelectMany(n => n.SectionProfileInfo.Content.TrustSignatures).ToList();
                ...
                foreach (var trustSignature in sortList)
                {
                    ...
                    if (!dic.TryGetValue(trustSignature, out sectionProfileInfo)) continue;

                    var tempItem = new SignatureTreeItem(sectionProfileInfo);
                    workSignatureTreeItems.Add(tempItem);

                    var target = checkedSignatureTreeItems.FirstOrDefault(n => n.SectionProfileInfo.Content.TrustSignatures.Contains(trustSignature));
